...
import { GetTrainerReservesUseCase } from "../../application/usecases/GetTrainerReservesUseCase";
...
export class GetTrainerReservesController {
  ...
  async handle(req: Request, res: Response) {
    ...
    try {
      ...
      res.status(201).send({ message: "Obtained all reserves of a trainer", reserves });
    } catch (error) {
